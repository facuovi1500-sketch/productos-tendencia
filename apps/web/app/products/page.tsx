    ...
    <>
      ...
      <DataTable<ProductRow>
        ...
        columns={[
          ...
          { key: "mode", label: "Modalidad", render: (row) => <Badge tone={row.mode === "STOCK" ? "blue" : "purple"}>{statusLabel(String(row.mode))}</Badge> },
          { key: "status", label: "Estado", render: (row) => <Badge tone={productStatusTone(String(row.status))}>{statusLabel(String(row.status))}</Badge> },
          {
            key: "doNotReorder",
            label: "Recompra",
            render: (row) => <Badge tone={row.doNotReorder ? "red" : "green"}>{row.doNotReorder ? "No recomprar todavía" : "OK para seguir vendiendo"}</Badge>,
          },
          {
            key: "reorderBlockReason",
            label: "Motivo de bloqueo",
            render: (row) => row.reorderBlockReason ? <span className="text-sm text-slate-700">{row.reorderBlockReason}</span> : "Sin bloqueo",
          },
        ]}
      />
    </>
